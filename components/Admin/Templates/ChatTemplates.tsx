...
import {
  useDeleteChatTemplateTemplate,
  useGetChatTemplateLists,
} from "@/hooks/admin";
...
export default function ChatTemplates() {
  ...
  const {
    ...
    search,
    setSearch,
  } = useGetChatTemplateLists(currentPage);
  const { chatTemplateDeleteHandler, isLoading: isDeleteProcessing } =
    useDeleteChatTemplateTemplate();
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: t("Template Name"), accessor: "name" },
      {
      ...
      },
      {
        Header: t("Help With"),
        accessor: (data: any) =>
          data.help_with.length > 100
            ? `${data.help_with.slice(0, 100)}...`
            : data.help_with,
      },
      // {
      //   Header: "Update At",
      ...
      // },
      {
        ...
        accessor: (data: any) => (
          <div>
            <Tippy content="Edit">
              <Link href={`/admin/templates/chat-templates/edit/${data.id}`}>
                <button type="button">
                  <svg
                    width="24"
                    height="24"
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="mb-4 items-center justify-between md:flex">
        <Link
          href={`/admin/templates/chat-templates/create`}

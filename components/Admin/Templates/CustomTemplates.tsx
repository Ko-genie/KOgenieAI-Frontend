...
export default function CustomTemplates() {
  ...
  const {
    ...
    search,
    setSearch,
  } = useGetCustomTemplateLists(currentPage);
  const { customTemplateDeleteHandler, isLoading: isDeleteProcessing } =
    useDeleteCustomTemplate();
  ...
  const columns = useMemo(
    () => [
        ...
        accessor: (data: any) => (
          <div
            className={`whitespace-nowrap ${
              data.package_type === 1 ? "text-primary" : "text-secondary"
            }`}
          >
            {data.package_type === 1 ? "Regular" : "Premium"}
          </div>
        ),
      },
      { Header: t("Template Name"), accessor: "title" },
      {
        Header: t("Template Description"),
        accessor: (data: any) =>
      ...
      },
      {
        ...
        accessor: (data: any) => data.templateCategory.name,
      },
      {
        ...
        accessor: (data: any) => (
          <div
            className={`whitespace-nowrap ${
              data.templateCategory.status === 1
                ? "text-success"
                : "text-danger"
            }`}
          >
            {data.templateCategory.status === 1 ? "Active" : "In-Active"}
          </div>
        ),
      },
      {
      ...
      },
      {
        ...
        accessor: (data: any) => (
          <div>
            <Tippy content="Edit">
              <Link href={`/admin/templates/custom-templates/edit/${data.id}`}>
                <button type="button">
                  <svg
                    width="24"
                    height="24"
  ...
  const deleteHandler = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (result.isConfirmed) {
          customTemplateDeleteHandler(id);
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="mb-4 items-center justify-between md:flex">
        <Link
          href={`/admin/templates/custom-templates/create`}

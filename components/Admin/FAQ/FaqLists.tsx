...
export default function FaqLists() {
  ...
  const { data: templateLists, isLoading } =
    useGetFaqListsForAdmin(currentPage);

  const { faqListsForAdminDeleteHandle, isLoading: isDeleteProcessing } =
    useDeleteFaqListsForAdmin();
  ...
  const deleteHandler = (id: any) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (result.isConfirmed) {
          faqListsForAdminDeleteHandle(id);
  ...
  const columns = useMemo(
    () => [
      { Header: t("Question"), accessor: "question" },

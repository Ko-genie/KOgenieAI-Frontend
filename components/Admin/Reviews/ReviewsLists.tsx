...
import {
  useDeleteReviewsListsForAdmin,
  useReviewsListsForAdmin,
} from "@/hooks/admin";
...
export default function ReviewsLists() {
  ...
  const {
    ...
    search,
    setSearch,
  } = useReviewsListsForAdmin(currentPage);

  const { reviewsListsForAdminDelete, isLoading: isDeleteProcessing } =
    useDeleteReviewsListsForAdmin();
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: t("User Name"), accessor: "user_name" },
      ...
      },
      {
        ...
        accessor: (data: any) => (
          <div>
            <Tippy content="Edit">
              <Link href={`/admin/site-settings/reviews/edit/${data.id}`}>
                <button type="button">
                  <svg
                    width="24"
                    height="24"
  ...
  return (
    <div>
      <div className="mb-4 items-center justify-between md:flex">
        <Link
          href={`/admin/site-settings/reviews/create`}
      ...
      <div className="panel h-full">
        <DynamicTable
          data={reviewLists?.data?.list || []}
          loading={isLoading}
          columns={columns}
          totalItems={reviewLists?.data?.meta?.total}
          perPageItems={reviewLists?.data?.meta?.perPage}
          handlePageClick={handlePageClick}
          onPageChange={setCurrentPage}
          activePage={reviewLists?.data?.meta?.currentPage}
        />
      </div>
    </div>
  );
